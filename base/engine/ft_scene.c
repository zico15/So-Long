...
/*   Updated: 2022/04/25 22:50:49 by edos-san         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_updade_scene(int fps)
{
	...
	while (ob)
	{
		if (ob->sprite.size && ob->type != PLAYER && ob->type != EXIT)
			ft_render_animation(ob);
		else if (ob->type == PLAYER)
			player = ob;
	...
	if (!player || !door)
		return (fps);
	ft_render_animation(player);
	ft_render_animation(door);
	return (fps);
}
